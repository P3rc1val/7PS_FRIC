...
function createData(taskName, system, analyst, priority, progress, numberSubtasks, numberFindings, duedate) {
  return {
    taskName,
    system,
    analyst,
...
function fillTableTask(props) {
  ...
  var data = [];
  taskData.map(m => data.push(createData(m.taskName, m.system, m.analyst, m.priority, m.progress, m.numberSubtasks, 
...
export default function EnhancedTable(props) {
  ...
  const handleClick = (event, taskName) => {
    const selectedIndex = selected.indexOf(taskName);
    let newSelected = [];
    if (selectedIndex === -1) {
      newSelected = newSelected.concat(selected, taskName);
  ...
  const isSelected = (taskName) => selected.indexOf(taskName) !== -1;
            ...
            >
              ...
              <TableBody>
                {stableSort(fillTableTask(props), getComparator(order, orderBy))
                  ...
                  .map((rows, index) => {
                    const isItemSelected = isSelected(rows.taskName);
                    const labelId = `enhanced-table-checkbox-${index}`;

                    return (
                      <TableRow
                        hover
                        onClick={(event) => handleClick(event, rows.taskName)}
                        role="checkbox"
                      ...
                      >
                        ...
                        <TableCell align="right">{rows.system}</TableCell>
                        <TableCell align="right">{rows.analyst}</TableCell>
                        <TableCell align="right">{rows.priority}</TableCell>
                        <TableCell align="right">{rows.progress}</TableCell>
                        <TableCell align="right">{rows.numberSubtasks}</TableCell>
                        <TableCell align="right">{rows.noFindings}</TableCell>
                        <TableCell align="right">{rows.duedate}</TableCell>
                      </TableRow>
                    );
                  })}
                {emptyRows > 0 && (
