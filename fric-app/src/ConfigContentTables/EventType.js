...
export default function EnhancedTable(props) {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = rows.map((n) => n.eventType);
  ...
  const handleClick = (event, eventType) => {
    const selectedIndex = selected.indexOf(eventType);
    let newSelected = [];

    if (selectedIndex === -1) {
      newSelected = newSelected.concat(selected, eventType);
  ...
  const isSelected = (eventType) => selected.indexOf(eventType) !== -1;
                ...
                {stableSort(fillTableSystem(props), getComparator(order, orderBy))
                  ...
                  .map((row, index) => {
                    ...
                    return (
                      <TableRow
                        hover
                        onClick={(event) => handleClick(event, row.eventType)}
                        role="checkbox"
